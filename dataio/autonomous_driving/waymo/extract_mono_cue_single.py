...
if __name__ == "__main__":
    ...
    parser.add_argument('--scene_id', type=str, default=None, help='specify --')
    ...
    if args.task == 'depth':
        output_dir = os.path.join(args.data_root, scene_id, args.depth_dirname)
    elif args.task == 'normal':
        output_dir = os.path.join(args.data_root, scene_id, args.normals_dirname)
    else:
        raise RuntimeError(f"Invalid task={args.task}")
    os.makedirs(output_dir, exist_ok=True)
      
    flist = sorted(glob(os.path.join(img_dir, '*.png')))
    for fpath in tqdm(flist, f'scene'):
